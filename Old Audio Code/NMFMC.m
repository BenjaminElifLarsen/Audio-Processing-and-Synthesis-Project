function [ CostMC,W,H,Spectrogram ] = NMF( S,W,H,NumberOfAtoms,Beta, NumberOfInterations,MonteCarlosAmount )
...
WHatMC = cell(1,MonteCarlosAmount);
HHatMC = cell(1,MonteCarlosAmount);
for i = 1:MonteCarlosAmount
    i
...
siz = size(V);
if Beta == 1 %KL
    cost = (sum(V(:).*log(V(:)./VHat(:))+(VHat(:)-V(:))));
elseif Beta == 0 %IS
    cost = (sum(V(:)./VHat(:)-log(V(:)./VHat(:))))-(siz(1)*siz(2));
end
